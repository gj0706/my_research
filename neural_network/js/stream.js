...
d3.json('data/model/layer1_10cells.json').then(function(data1){
    d3.json('data/model/layer2_10cells.json').then(function(data2){
        ...
        for(let j = 0; j < 8; j++){
            for(let i = 0; i < 10; i++){
                draw_single_chart(data1, `#stream${j+1}`, j, i);
            }
        };
        // draw layer 2
        for(let j = 0; j < 10; j++){
            for(let i = 0; i < 10; i++){
                draw_single_chart2(data2, `#c${j+1}`, j, i);
            }
        };
    });
});
function draw_single_chart2(data, selector, l1Cell, l2Cell){
    // Data should be per cell per feature
    let filtered = data.filter(d=>d["cell_l1"] === l1Cell && d["cell_l2"] === l2Cell);
